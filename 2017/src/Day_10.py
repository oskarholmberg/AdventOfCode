...
def sparse_knot_hash(_list, _lengths, skip_size=0, current_position=0, laps=1):
    lap_count = 0
...
def knot_hash(string):
    ascii_lengths = [ord(c) for c in list(string)]
    ascii_lengths.extend([17, 31, 73, 47, 23])

    sparse_hash = sparse_knot_hash(_list=list(range(0, 256)), _lengths=ascii_lengths, laps=64)

    dense_hash = []
    for i in range(0, len(sparse_hash), 16):
        dense_hash.append(reduce(lambda i, j: int(i) ^ int(j), sparse_hash[i:i+16]))
    return "".join(["{0:0{1}x}".format(i, 2) for i in dense_hash])


def run():
    input_string = open("../inputs/Day_10.txt").readline()
    lengths = input_string.split(",")

    _hash = sparse_knot_hash(_list=list(range(0, 256)), _lengths=lengths)

    print(f"Part 1: {_hash[0]*_hash[1]}")

    print("Part 2:", knot_hash(input_string))
